<?php
	...
	{	$ind = 3;
	  ...
	  else
	  {
		 ...
		 $i=0;
		 $responsetext.="<div id='div2'><h3>Team ".$_SESSION['id']."!    The results are...</h3><table style='width:97%;margin-left:20px;margin-top:50px;position:relative;'><tr><th>Team ID</th><th>Team Name</th><th>Result</th></tr>";
		 while($i<$rows)
		 {  
		    mysqli_data_seek ( $res,$i );
			$ans = mysqli_fetch_assoc($res);
			if($ans['teamName'][$i]==$_SESSION['id'])
			$responsetext.="<tr><td>".$ans['teamID']."</td><td>".$ans['teamName']."</td><td>".$ans['teamResult']."</td></tr>";
			else
			$responsetext.="<b><tr><td>".$ans['teamID']."</td><td>".$ans['teamName']."</td><td>".$ans['teamResult']."</td></tr></b>";
			$i=$i+1;
